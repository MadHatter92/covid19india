...
for state, pop in zip(state_list, state_pop):
	state_data = data.get(state)
	latest = state_data.get(list(state_data)[-30])
...
color = ['green' if end-beginning <= 0.0 else 'red' for end, beginning in zip(deceased_2, deceased_1)]

df = pd.DataFrame(({'State':state_list, 'Beginning': deceased_1, 'End': deceased_2, 'Color':color}))
...
plt.scatter(ordered_df['Beginning'], my_range, color = 'grey', alpha = 0.3, label = '30 Days Ago')
...
plt.xlabel('Percentage deceased out of total confirmed')
plt.title('COVID-19 Statewise Progress: Total deceased out of total confirmed')
...
for y, Beginning, End in zip(my_range, ordered_df['Beginning'], ordered_df['End']):
	...
	plt.annotate("{:.1%}".format(End), (End, y), ha='right',size=9, weight = 'bold')
